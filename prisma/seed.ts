...
async function main() {
  ...
  // Supprimer tous les événements existants (optionnel)
  await prisma.eventoEvent.deleteMany();
  console.log("Deleted existing events");

  // Créer les événements sans spécifier l'ID
  for (const event of events) {
    const result = await prisma.eventoEvent.create({
      data: {
        ...event,
        date: new Date(event.date), // S'assurer que la date est un objet Date
      },
    });
